...
sorted_disks sort_alternate(const disk_state& before) {
    ...
    {
        for (size_t j = i%2; j < after.total_count()-1; j+= 2)
        {
...
sorted_disks sort_lawnmower(const disk_state& before) {
    ...
    for(size_t k = 0; k <= after.total_count()/4; k++)
    {
        for (size_t i = k; i < after.total_count()-1;i++)
        {
        ...
        for (size_t j = after.total_count()-1; j > k; j--)
        {
